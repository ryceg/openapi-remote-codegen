...
function generateInvalidationUtilsFile(): string {
  return `// AUTO-GENERATED - DO NOT EDIT
// Generated by openapi-remote-codegen
...
export async function refreshInvalidated(
  ...
  const outcomes = await Promise.allSettled(refreshes.map((refresh) => refresh()));

  for (const outcome of outcomes) {
    if (outcome.status === 'rejected') {
      console.error(\`Invalidation refresh failed after \${operation}:\`, outcome.reason);
    }
  }
}
`;
...
function generateTagFile(
  ...
  const functions = operations
    .map(op => generateFunction(op, tag, invalidation, config))
    .join('\n\n');

  // Only import query/command/form if actually used. File uploads always emit
  // form(), whatever remote type they declare, so they move a tag off command().
  const hasQuery = operations.some(op => op.remoteType === 'query');
  const hasCommand = operations.some(op => op.remoteType === 'command' && !op.isFileUpload);
  const hasForm = operations.some(op => op.remoteType === 'form' || op.isFileUpload);
  const serverImports = [
    'getRequestEvent',
    ...(hasQuery ? ['query'] : []),
    ...(hasCommand ? ['command'] : []),
    ...(hasForm ? ['form'] : []),
    ...(invalidation.usesRequested ? ['requested'] : []),
  ];
  ...
  const usesFormCoerce = operations.some(op => op.remoteType === 'form' && !op.isFileUpload);
  const usesZodForForms = usesFormCoerce || usesZod;
  ...
  const invalidationImport = invalidation.usesRequested
    ...
    : '';
  const crossTagImports = invalidation.importLines.length > 0
    ? `${invalidation.importLines.join('\n')}\n`
    : '';
...
function generateFunction(
  ...
  if (op.remoteType === 'query') {
    ...
    return generateMutationFunction(op, functionName, clientProperty, methodName, refreshCalls, 'form', config);
  } else {
    return generateMutationFunction(op, functionName, clientProperty, methodName, refreshCalls, 'command', config);
  }
}
...
function generateRefreshCalls(
  ...
  const targets = invalidation.targetsByOperationId.get(op.operationId) ?? [];
  const thunks = refreshThunks(op, targets);
  if (thunks.length === 0) return '';

  return `\n    await refreshInvalidated('${functionName}', [
      ${thunks.join(',\n      ')}
    ]);`;
}
 ...
 *
 * The wrapper is a `form()` remote and the caller submits a `<form>` carrying a file
 * input named after {@link OperationInfo.fileFieldName}; see the note on `wrapperName`
 * for why a `command()` cannot work here.
 */
function generateFileUploadFunction(
  ...
  // Always form(), even when the operation declares itself a command: command
  // arguments are devalue-serialised and devalue cannot represent a File, so a
  // command wrapper throws in the browser before any request is sent. Form
  ...
  return `${comment}export const ${functionName} = ${wrapperName}('unchecked', async (data: { ${fieldName}?: File }) => {
