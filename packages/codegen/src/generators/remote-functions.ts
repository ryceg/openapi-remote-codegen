...
function generateTagFile(tag: string, operations: OperationInfo[], config: GeneratorConfig): string {
  ...
  // form() receives FormData where all values are strings. z.fromJSONSchema() produces
  // strict validators (z.boolean() rejects "true", z.string().date() rejects "").
  // Emit a preprocess wrapper that coerces common FormData string types.
  const formCoerceHelper = hasForm ? `
...
${zodImportLine}${schemaImportLine}${apiImportLine}${formCoerceHelper}
${functions}
`;
}
